...
# https://github.com/dangnh0611/kaggle_rsna_breast_cancer/blob/reproduce/src/utils/dicom.py
...
# "dicom sdl" allows to make programs that read DICOM formatted files without in depth knowledge of DICOM.
# pip3 install dicomsdl
import dicomsdl
...
# NVIDIA Data Loading Library (DALI) is a library for data loading and pre-processing to accelerate deep learning applications.
...
class DicomsdlMetadata:
    ...
    def __init__(self, ds):
        ...
        # invert => A boolean flag indicating whether the image needs to be inverted based on the photometric interpretation.
        self.invert = (ds.PhotometricInterpretation == 'MONOCHROME1')
        assert len(self.window_widths) == len(self.window_centers)
...
def _convert_single_with_dicomsdl(dcm_path,
    ...
    meta = DicomsdlMetadata(dcm)# class DicomsdlMetadata 
...
def convert_with_dicomsdl_parallel(dcm_paths,
                                   ...
                                   parallel_n_jobs=2,
                                   joblib_backend='loky',
                                   # Specifies the backend (from joblib library) for parallel processing (default: 'loky').
                                   legacy=False):
    '''
        This code snippet performs parallel conversion of DICOM  files using the DicomSDL library.
        Parallel and delayed functions are part of the joblib library, which is commonly used for parallel computing in Python.
    '''
    assert len(dcm_paths) == len(save_paths)
    if parallel_n_jobs == 1:
        print('No parralel. Starting the tasks within current process.')
        ...
        _ = Parallel(n_jobs=parallel_n_jobs, backend=joblib_backend)( 
                ...
                (dcm_paths[j], save_paths[j], normalization=normalization, save_backend=save_backend, 
                 save_dtype=save_dtype, legacy=legacy) for j in tqdm(range(len(dcm_paths)))
                # _convert_single_with_dicomsdl function is called for each pair of "dcm_paths" and "save_paths".
                                                                     
        )# end of ....joblib_backend)(
